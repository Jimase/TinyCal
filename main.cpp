...
public:
    ...
    SYMBOL
    make_symb(string sym, int typ, int pri, int pos = 0, double val = 0) {
        SYMBOL
                res;
        if (typ == 6) {
            res.sym = "$string";
    ...
    void
    make_symb_and_push(string sym, int typ, int pri, int pos = 0, double val = 0) {
    ...
    init_buildin_symbols() {
        ...
        make_symb_and_push("Pi", 4, -1, 0, 3.1415926535897932384626433832795);
        make_symb_and_push("E", 4, -1, 0, 2.7182818284590452353602874713527);
    ...
    next_token(int &x, string &expr) {
        ...
        if (in_string(expr[x], "+-*/=!%^&<>(),|")) {
            for (; x < expr.size(); x++)
                for (int i = 0; i < registered_symbols.size(); i++) {
                    ...
                    for (int j = 0; j < registered_symbols[i].sym.size(); j++)
                        if (registered_symbols[i].sym[j] == expr[j + x])
                            ...
                            break;
                    if (cnt == registered_symbols[i].sym.size())
                        return
                                i;
                }
            return
        ...
        for (; x < expr.size() && token_t(expr[x]); x++)
    ...
    get_string(string &expr, int &addr) {
        ...
        if (expr[addr] == '\"')
            addr++;
        for (; addr < expr.size(); addr++) {
            if (expr[addr] == '\"') {
                addr++;
                break;
            }
            if (expr[addr] == '\\' && addr < expr.size() - 1) {
                if (expr[addr + 1] == '\"') {
                    res.push_back('\"');
                    addr++;
                    continue;
    ...
    print_err() {
        for (int i = 0; i < err_msg.size(); i++) {
            cout << "Pos " << err_msg[i].first << ' ' << err_msg[i].second << endl;
        }
    }
    ...
    lexer(vector<SYMBOL> &tokens, string &expr) {
        ...
        for (int i = 0; i < expr.size();) {
            ...
            for (; i < expr.size() && expr[i] == ' '; i++);  //Reduce Space
        ...
        if (test_mode) {
            ...
            for (int i = 0; i < tokens.size(); i++)
                cout << '	' << tokens[i].sym << '	' << tokens[i].typ << '	' << tokens[i].pos << ' '
                     << tokens[i].val << endl;
            cout << "End of line\n";
        }
    ...
    executer(vector<SYMBOL> &tokens, vector<SYMBOL> &res) {
        ...
        stack<double> integrate_result;
        stack<double> integrate_last;

        stack<int> diff_state;
        stack<double> diff_result;
        ...
        stack<int> sum_state;
        stack<double> sum_result;

        stack<int> quadrature_state;
        stack<double> quadrature_result;
        ...
        for (int i = 0; i < tokens.size(); i++) {
            ...
            if (test_mode) {
                cout << "Now token = " << i << endl;
                cout << "Now the top of val_stack is\n";
                if (!val_stack.empty())
                    print_list(val_stack.top());
                else
                    cout << "NULL" << endl;
                cout << endl;
                cout << "Now the top of opr_stack is\n";
                if (!opr_stack.empty())
                    cout << opr_stack.top().sym << ' ' << opr_stack.top().typ << endl;
                else
                    cout << "NULL" << endl;
                cout << endl;
            }
                    ...
                    while (!opr_stack.empty() &&
                            ...
                            for (int j = 0; j < max(a.size(), b.size()); j++) {
                                ...
                                if ((a[ia].typ == 0) && (b[ib].typ == 0) ||
                                    ...
                                    if (temp.sym == "+") {
                                        ...
                                        err_msg.push_back(make_pair(temp.pos, "Error : \"" + temp.sym +
                                                                              "\" is an unknow symbol."));
                                    if (flag)
                                        c.push_back(a[ia]);
                                    else
                                    ...
                                    if (temp.sym == "+")
                                        ...
                                        err_msg.push_back(
                                                make_pair(temp.pos, "Error : " + temp.sym + " : Unknow symbol."));
                                    if (flag)
                        ...
                        if (temp.typ == 3) {
                            ...
                            if (temp.sym == "Sort") {
                                ...
                                if (!flag) {
                                    ta = --vars[vars.size() - 1][a[0].sym].val;
                                    a.clear();
                                    a.push_back(make_symb("$number", 0, -1, -1, ta));
                                }
                                ...
                                if (!flag) {
                                    if (solve_loop.top() == max_solve_loop)
                                        vars[vars.size() - 1][a[0].sym].val = 0.01 * (rand() % 100);
                                    ...
                                    if (ic <= 0) {
                                        ...
                                        if (ic == max_integrate_loop) {
                                            ...
                                            if (!integrate_last.empty()) {
                                                tc += (td + integrate_last.top()) * ta * 0.5;
                                                integrate_last.pop();
                                            }
                                    ...
                                    if (a[j].typ != 9 && a[j].typ != 5)
                                        ...
                                        if (is_number(sa[0])) {
                                            ...
                                            vars[vars.size() - 1][a[j].sym].typ = 5;
                                                   ...
                                                   (is_number(sa[1]) && (sa[0] == '-' || sa[0] == '+'))) {
                                            ia++;
                                            vars[vars.size() - 1][a[j].sym].val =
                                                    get_number(sa, ia) * (sa[0] == '-' ? -1.0 : 1.0);
                                            vars[vars.size() - 1][a[j].sym].typ = 5;
                                        } else {
...
main() {
    ...
    bool
            flag = 0;
    //intp.test_mode_on();
    //MessageBox(NULL,"Kernel started.","TinyCalc Kernel",MB_ICONINFORMATION|MB_OK);
    cout << "TinyCalc kernel [1.0.4] By Cansanel\n\n";
    while (1) {
        if (!flag)
            cout << ">>> ";
        else
